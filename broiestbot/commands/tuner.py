...
from config import CHANNEL_HOST,CHANNEL_AUTH,CHATANGO_SPECIAL_USERS
...
def tuner(name, user):
    ...
    #some of this has to use ugly plus signs because format() breaks due to all the curlies    
        ...
        data2 = '{"id":752,"jsonrpc":"2.0","method":"PVR.GetBroadcasts","params":{"channelid":'+number+',"properties":["isactive","starttime","endtime","title"], "limits":{ "end": 2}}}'
        ...
        return emojize(
            f":tv: Tuning to {capped}. On now: {onnow}",
            use_aliases=True
        )
    return emojize(
